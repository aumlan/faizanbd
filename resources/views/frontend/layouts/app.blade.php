...
<link rel="stylesheet" href="{{ asset('css/bootstrap-datepicker.min.css') }}" type="text/css" media="none" onload="if(media!='all')media='all'">
...
<body>
    {{-- <nav class="fix-side-menu">
        <ul>
            <li><a href=""><span><i class="la la-retweet d-inline-block nav-box-icon"></i></span>You Know</a></li>
...
<script>
    ...
    function addToListCart(key){
        ...
        if(checkAddToCartValidity()) {
            $('#addToCart').modal();
            $('.c-preloader').show();
            $.post('{{ route('cart.addToCart') }}', { _token : '{{ @csrf_token() }}', id : key}, function(data){
                   $('#addToCart-modal-body').html(null);
                   $('.c-preloader').hide();
                   $('#modal-size').removeClass('modal-lg');
                   $('#addToCart-modal-body').html(data);
