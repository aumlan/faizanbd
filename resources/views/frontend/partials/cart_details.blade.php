            ...
            <div class="form-default bg-white p-4">
                        ...
                        <table class="table-cart border-bottom">
                            <thead>
                                <tr>
                                    ...
                                    @auth
                                    @if(auth()->user()->user_type=='seller')
                                    <th style="    padding-left: 15px;" class="product-price  d-lg-table-cell">{{__('Profit')}}</th>
                                    @endif
                                    @endauth
                                ...
                                @foreach (Session::get('cart') as $key => $cartItem)
                                    ...
                                    <tr class="cart-item">
                                        <td class="product-image  d-lg-block">
                                            <a href="#" class="mr-3">
                                                <img style="width: 45px" loading="lazy"  src="{{ asset($product->featured_img) }}">
                                            </a>
                                        </td>
                                        ...
                                        @if(auth()->user()->user_type=='seller')
                                            ...
                                            <span style="    padding-left: 15px;">{{ single_price(($cartItem['price']*$cartItem['quantity'])-($cartItem['wp']*$cartItem['quantity'])) }}</span>
                                        </td>
                                            @endif
                                            @endauth
                ...
                <div class="row align-items-center pt-4">
                    ...
                    <div class="col-6 text-right">
                        ...
                        @else
                            <a href="{{ url('checkout') }}"><button class="btn btn-styled btn-base-1">{{__('Continue to Shipping')}}</button></a>
                        @endif
                    </div>
                </div>
            </div>
