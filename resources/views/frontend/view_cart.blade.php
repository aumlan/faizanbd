                    ...
                    <div class="form-default bg-white p-4">
                                        ...
                                        @foreach (Session::get('cart') as $key => $cartItem)
                                            ...
                                            <tr class="cart-item">
                                                <td class="product-image d-lg-block">
                                                    <a href="#" class="mr-3">
                                                        <img style="width: 50px" loading="lazy"  src="{{ asset($product->featured_img) }}">
                                                    </a>
                                                </td>
                                                ...
                                                <td class="product-price d-none d-lg-table-cell">
                                                    <span class="pr-3 d-block">{{ single_price($cartItem['wp']) }}</span>
                                                </td>
                                            ...
                                            @if(auth()->user()->user_type=='seller')
                                                ...
                                                <td class="product-price d-lg-table-cell">
                                                    <span class="pr-3 d-block">
                                                        <input style="padding: 5px;width: 100px" type="text" name="price[{{ $key }}]" class="form-control input-number" placeholder="1" value="{{ $cartItem['price'] }}" min="1" max="10000" onchange="updatePrice({{ $key }}, this)">
                                                    </span>
                                                </td>
                                            @endif
                                            @endauth
                        ...
                        <div class="row align-items-center pt-4">
                            ...
                            <div class=" col-6 col-md-6 col-sm-6 text-right">
                                @if(Auth::check())
                                    <a href="{{ route('checkout.shipping_info', 'inside_dhaka') }}" class="btn btn-styled btn-base-1">{{__('Continue to Shipping')}}</a>
                                @else
                                    <a href="{{ route('checkout.shipping_info', 'inside_dhaka') }}"><button class="btn btn-styled btn-base-1">{{__('Continue to Shipping')}}</button></a>
                                @endif
                            </div>
                        </div>
                    </div>
                             ...
                             @if (\App\Addon::where('unique_identifier', 'otp_system')->first() != null && \App\Addon::where('unique_identifier', 'otp_system')->first()->activated)
                                            <div class="form-group phone-form-group">
                                                <div class="input-group input-group--style-1">
                                                    <input style="    padding: 13px;" type="tel" id="phone-code" class="border-right-0 h-100 form-control{{ $errors->has('phone') ? ' is-invalid' : '' }}" value="{{ old('phone') }}" placeholder="{{ __('e.g. 8801799388112
') }}" name="phone">
...
@section('script')
    ...
    function updatePrice(key, element){
        $.post('{{ route('cart.updatePrice') }}', { _token:'{{ csrf_token() }}', key:key, price: element.value}, function(data){
            updateNavCart();
