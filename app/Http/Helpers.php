...
if (! function_exists('sendSMS')) {
    function sendSMS($to, $from, $text)
    {
        ...
        elseif (OtpConfiguration::where('type', 'smsbd')->first()->value == 1) {

            // $user = env("SMSBD_USER");
            // $pass = env("SMSBD_TOKEN");
            // $url = env("SMSBD_URL");
            // $param = "api_token=mESvXiklsEKsUPAw2lUOgYVkP4EIizxac0MUPYng&senderid=skyitdev.com/lazma&message=". urlencode($text) ."&contact_number=".$to;
            // $crl = curl_init();
            // curl_setopt($crl, CURLOPT_SSL_VERIFYPEER, FALSE);
            // curl_setopt($crl, CURLOPT_SSL_VERIFYHOST, 2);
            // curl_setopt($crl, CURLOPT_URL, $url);
...
// reseller base
if (! function_exists('home_reseller_price')) {
    function home_reseller_price($id)
    {
        $product = Product::findOrFail($id);
        $lowest_price = $product->unit_price;
        $highest_price = $product->unit_price;
...
//Shows Base Price with reseller price
if (! function_exists('home_reseller_discounted_base_price')) {
    function home_reseller_discounted_base_price($id)
    {
